...
def handle_outcomes(stdout_expected, stdout_actual, stderr_expected, stderr_actual, rc_expected, rc_actual,
                    fout_name, fout_actual, fout_expected, verbosity, total_time):
    test_passed = True
...
def test_one(test_number, input_dir, timeout_length, verbosity, show_timing):
    ...
    if cmp_fout:
        ...
        for f in fout_name:
            fout_expected.append(read_file('%s/%s' % (input_dir, f), do_binary))
    ...
    if cmp_fout:
        for f in fout_name:
            fout_actual.append(read_file('%s' % f, do_binary))

    return handle_outcomes(stdout_expected, stdout_actual, stderr_expected, stderr_actual, rc_expected, rc_actual,
                           fout_name, fout_actual, fout_expected, verbosity, total_time), score
...
sys.stderr.write(f'TEST SCORE: {test_score} / {total_score}\n\n')
exit(0)
